...
patents/                  Bibliographic filing and status record
...
The repository does not contain patent specifications, filing receipts, administrative patent documents, internal claim-support analysis, private correspondence, confidential evidence, proprietary source code, tests, policy packs, or private implementation records.

## Status boundaries

The architecture documents distinguish among:

- **architectural invariants**, which define the governance model;
- **capability statements**, which describe the architecture's defined scope but do not claim that every deployment implements every surface; and
- **reported runtime structure**, which records public implementation vocabulary as at the stated date without publishing the proprietary implementation.

Patent applications are not granted patents. Publication and market activity are not evidence of inventorship. Provenance claims in this repository depend on the cited record, not on commercial uptake or institutional recognition.
